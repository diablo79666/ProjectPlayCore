...
// Pfad: /backend/tools/admin_buttons_insert_cleanup.php
// Zweck: Fügt den Cleanup-Button robust ein (prüft Duplikate nach Titel & Pfad)
// ============================================================================
...
try {
    // ------------------------------------------------------------------------
    // Spalten prüfen (Kompatibilität mit älteren DB-Schemata)
    // ------------------------------------------------------------------------
    $cols = [];
    ...
    // Spaltenname für Gruppe dynamisch ermitteln
    ...
    // Zielwerte
    // ------------------------------------------------------------------------
    $title  = 'Container Health & Cleanup';
    $href   = '/backend/tools/container_health_cleanup.php';
    $area   = 'dashboard';
    $group  = 'System';
    $sort   = 120;
    $cap    = 'view_admin';
    ...
    // ------------------------------------------------------------------------
    // Duplikate prüfen (nach Titel oder Pfad)
    // ------------------------------------------------------------------------
    ...
    if ($existingId) {
        ...
        $msg = "♻️ Bestehender Button wurde aktualisiert (ID $existingId).";
    } else {
        // Neu einfügen
        $ins = $db->prepare("
        ...
        $msg = "✅ Neuer Button wurde erfolgreich hinzugefügt.";
    }

    // ------------------------------------------------------------------------
    // Cleanup: doppelte oder fehlerhafte Einträge automatisch entfernen
    // ------------------------------------------------------------------------
    $cleanup = $db->prepare("
